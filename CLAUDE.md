...
- Pre-rendered content generated by `build-prerender.py` (Python) — idempotent (containers can be re-generated in place); also emits minified `*.min.js` / `*.min.css` copies (rjsmin/rcssmin) that the HTML pages reference — never edit `.min` files by hand, edit sources and re-run the build
...
# Regenerate pre-rendered content + minified assets (run after ANY edit to
...
- **8 images**: social share card (og-share.jpg, 1200×630 landscape, listed first) + primary photo (IMG-2.jpg) + CANS lab logo + distinguished alumni + education award + EpiRank + Laosong 130th anniversary alumni award + Laosong donation — each with `og:image:alt` and `og:image:width`/`height`
